...
with open('men_names.txt') as f:
    men_names = f.read().splitlines()
with open('women_names.txt') as f:
...
for i in range(0,100000):
    ...
    user = User(username=username, first_name=name, last_name=surname, email=email, password=pas)
    user.save()
times.append(time.time())
adjectives = ()
nouns = ()
...
for i in range(0,2000):
    ...
    shelter = Shelter(name=name, location=location, email=email)
    shelter.save()
times.append(time.time())
del men_names
del women_names
...
for i in range(0,200000):
    ...
    photo = str(random.randint(1,2)) + ".jpg"
    ...
    if random.randint(1,25) == 1:
        ...
        pet = Pet(name=name, ptype=ptype, sex=sex, photo=photo, in_date=in_date, 
                out_date=out_date, shelter_id=shelter_id, owner_id=owner_id)
        pet.save()
    else:
        pet = Pet(name=name, ptype=ptype, sex=sex, photo=photo, in_date=in_date, shelter_id=shelter_id)
        pet.save()
times.append(time.time())
del pet_names
times.append(time.time())
for i in range(0,100000):
    ...
    work = Volunteer_work(volunteer=volunteer, shelter=shelter, date=work_date,
...
for i in range(0,100000):
    ...
    if random.randint(0,1) == 0:
        ...
        rt = Rating(rating=rating, comment=comment, shelter=shelter, 
                content_object=user)
        rt.save()
    else:
